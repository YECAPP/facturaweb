...
//Creadno bucle de datos   
    ...
    foreach ($pdo->query($sql) as $row) 
    {
        ...
        $data.='<td><button onclick="loadProdData('.$row['IDPRODUCTO'].')" class="btn btn-warning">Mod</button></td>';
        $data.='<td><button onclick="deleteProd('.$row['IDPRODUCTO'].')" class="btn btn-danger">Borrar</button></td>';
    	$data.='</tr>';
    }
    $data.='</tbody>';
    $data.='</table>';
